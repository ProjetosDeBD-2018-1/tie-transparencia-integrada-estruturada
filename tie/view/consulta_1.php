...
<?php include (CONNECTION);
    ...
    $nRow = 0;
    try {
      $array_result = select_convenio_total_pago();
...
<script type="text/javascript">
  ...
  function drawBasic() {
    var data = google.visualization.arrayToDataTable([
      ...
      [ '<?=$row['NR_convenio']?>', <?=$row["diferenca_raw"]*(-1)?>, <?=br_string_to_float($row["totalempenho"])?>, <?=br_string_to_float($row["totalpago"])?> ],
      <?php } ?>
    ]);
    ...
    var options = {
      ...
      chartArea: {width: '65%', height: '98%', backgroundColor: '#2C3034'},
      height: <?=$total_row*30?>,
      // width: '100%',
      bar: {groupWidth: "90%"},
      hAxis: {title: 'Valor em Reais', minValue: 0, titleTextStyle: {color: 'white'}, textStyle: {color: 'white', fontSize: 14}, gridlines:{count:21}},
      vAxis: {title: 'Números dos Convênios', titleTextStyle: {color: 'white'}, textStyle: {color: 'white', fontSize: 14}},
